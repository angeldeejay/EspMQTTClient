...
EspMQTTClient::EspMQTTClient(
    ...
    const uint16_t mqttServerPort) : EspMQTTClient(wifiSsid, wifiPassword, mqttServerIp, NULL, NULL, mqttClientName, mqttServerPort)
{
}
...
EspMQTTClient::EspMQTTClient(
    ...
    const char *mqttPassword,
    const char *mqttClientName,
    const uint16_t mqttServerPort) : _wifiSsid(wifiSsid),
...
// Set static Wifi config
void EspMQTTClient::setWifiStaticConfig(IPAddress local_ip, IPAddress gateway, IPAddress subnet, IPAddress dns1, IPAddress dns2)
{
  _local_ip = local_ip;
  _gateway = gateway;
  _subnet = subnet;
  _dns1 = dns1;
  _dns2 = dns2;
  _useStaticWifiConfig = true;
}

// Set static Wifi config
void EspMQTTClient::setWifiStaticConfig(const char *local_ip, const char *gateway, const char *subnet, const char *dns1, const char *dns2)
{
  IPAddress __local_ip = IPAddress();
  IPAddress __gateway = IPAddress();
  IPAddress __subnet = IPAddress();
  IPAddress __dns1 = IPAddress();
  IPAddress __dns2 = IPAddress();

  __local_ip.fromString(local_ip);
  __gateway.fromString(gateway);
  __subnet.fromString(subnet);
  __dns1.fromString(dns1);
  __dns2.fromString(dns2);
  setWifiStaticConfig(__local_ip, __gateway, __subnet, __dns1, __dns2);
}
...
{
  WiFi.mode(WIFI_STA);
  if (_useStaticWifiConfig)
  {
    WiFi.config(_local_ip, _gateway, _subnet, _dns1, _dns2);
  }
#ifdef ESP32
